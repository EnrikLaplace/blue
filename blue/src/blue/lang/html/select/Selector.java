...
public class Selector {
    ...
    private Selector(String query, HtmlElement root) {
    ...
    private Selector(Evaluator evaluator, HtmlElement root) {
     ...
     */
    public static Elements select(String query, HtmlElement root) {
     ...
     * @return matching elements, empty if none
     */
    public static Elements select(Evaluator evaluator, HtmlElement root) {
     ...
     * @return matching elements, empty if none
     */
    public static Elements select(String query, Iterable<HtmlElement> roots) {
        Validate.notEmpty(query);
        Validate.notNull(roots);
        Evaluator evaluator = QueryParser.parse(query);
        ArrayList<HtmlElement> elements = new ArrayList<HtmlElement>();
        IdentityHashMap<HtmlElement, Boolean> seenElements = new IdentityHashMap<HtmlElement, Boolean>();
        // dedupe elements by identity, not equality

        for (HtmlElement root : roots) {
            final Elements found = select(evaluator, root);
            for (HtmlElement el : found) {
    ...
    static Elements filterOut(Collection<HtmlElement> elements, Collection<HtmlElement> outs) {
        Elements output = new Elements();
        for (HtmlElement el : elements) {
            boolean found = false;
            for (HtmlElement out : outs) {
                if (el.equals(out)) {
                    found = true;
                    break;
                }
