...
public class TextNode extends Node {
	...
	void outerHtmlHead(Appendable accum, int depth, HtmlDocument.OutputSettings out) throws IOException {
        if (out.prettyPrint() && ((siblingIndex() == 0 && parentNode instanceof HtmlElement && ((HtmlElement) parentNode).tag().formatAsBlock() && !isBlank()) || (out.outline() && siblingNodes().size()>0 && !isBlank()) ))
            indent(accum, depth, out);

        boolean normaliseWhite = out.prettyPrint() && parent() instanceof HtmlElement
                && !HtmlElement.preserveWhitespace(parent());
        Entities.escape(accum, getWholeText(), out, false, normaliseWhite, false);
    }

	void outerHtmlTail(Appendable accum, int depth, HtmlDocument.OutputSettings out) {}
